...
impl Part1{
    ...
    fn check_wordsearch(&self, x:i32, y: i32) -> i32{
        let mut count = 0;
        if self.check_spot(x, y) == 'X'{
            if self.check_north(x, y) {count += 1}
            if self.check_north_east(x, y) {count += 1}
            if self.check_east(x, y) {count += 1}
            if self.check_south_east(x, y) {count += 1}
            if self.check_south(x, y) {count += 1}
            if self.check_south_west(x, y) {count += 1}
            if self.check_west(x, y) {count += 1}
            if self.check_north_west(x, y) {count += 1}
        }
        return count
    }

    fn check_west(&self, x:i32, y: i32) -> bool{
        if self.check_spot(x-1, y) == 'M' && self.check_spot(x-2, y)  == 'A' && self.check_spot(x-3, y) == 'S' {
            return true
        }
        return false
    }

    fn check_south_west(&self, x:i32, y: i32) -> bool{
        if self.check_spot(x-1, y+1) == 'M' && self.check_spot(x-2, y+2)  == 'A' && self.check_spot(x-3, y+3) == 'S' {
            return true
        }
        return false
    }
    ...
    fn check_south(&self, x:i32, y: i32) -> bool{
        if self.check_spot(x, y+1) == 'M' && self.check_spot(x, y+2)  == 'A' && self.check_spot(x, y+3) == 'S' {
            return true
        }
        return false
    }

    fn check_south_east(&self, x:i32, y: i32) -> bool{
        if self.check_spot(x+1, y+1) == 'M' && self.check_spot(x+2, y+2)  == 'A' && self.check_spot(x+3, y+3) == 'S' {
            return true
        }
        return false
    ...
    fn check_north_east(&self, x:i32, y: i32) -> bool{
        if self.check_spot(x+1, y-1) == 'M' && self.check_spot(x+2, y-2)  == 'A' && self.check_spot(x+3, y-3) == 'S' {
            return true
        }
        return false
    ...
    fn check_north_west(&self, x:i32, y: i32) -> bool{
        if self.check_spot(x-1, y-1) == 'M' && self.check_spot(x-2, y-2)  == 'A' && self.check_spot(x-3, y-3) == 'S' {
            return true
        }
        return false
